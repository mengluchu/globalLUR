...
plot_error = function(validation = 0, prediction = 0, geome, err = NA, extremequantil_low = 0.025, extremequantil_high = 0.975, plotraster = T) {
    ...
    if (!is.na(err)) {
        ...
        if (plotraster == F) 
            ggplot(sf1) + geom_sf(aes(color = error)) + scale_colour_gradientn(colours = colorG) + labs(title = "validation error") else {
            e <- extent(sf1)
        ...
        if (plotraster == F) 
            ggplot() + geom_sf(data = sf1, aes(color = error)) + scale_colour_gradientn(colours = colorS) + labs(title = "residuals") + layer(sp.polygons(Germany)) else {
            e <- extent(sf1)
            r <- raster(e, resolution = 0.5)
            x <- rasterize(sf1, r, fun = mean)
            rasterVis::levelplot(x[[2]], margin = F)
