...
create_ring = function(inde_var, normalize = F, buffers_in = c(0, 25, 50, 100, 300, 500, 800, 1000, 3000), buffers_out = c(25, 50, 100, 300, 500, 800, 1000, 3000, 5000), number_roadtypes, Road_varname = "ROAD") {
    ...
    # if(is.null(names(othervar))) names(othervar) = 'VAR1' roadsonlyin_s = roadsonly_in [,c(7,1:6,14, 8:13, 21, 15:20, 28, 22:27)] #shuffel, 0 is in the beginning, for buffer1 roadsonlyin_s = roadsonly_in [,c(for5000m[1],1:6,for5000m[2],
    # 8:13, for5000m[3], 15:20 )] roadsonlyin_s = roadsonly_in [,c(7,1:6,14, 8:13, 21, 15:20, 28, 22:27,35, 29:34)] roadrings = roadsonly - roadsonlyin_s
...
# devtools::create('globalLUR') load_all() document()
