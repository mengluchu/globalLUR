...
create_ring = function(inde_var,normalize=F, buffers_in = c(0,25,50,100,300,500,1000),
                       buffers_out = c(25, 50,100,300,500,1000,5000),number_roadtypes)

{
  buffernumber = length(buffers_in)
  roadsonly =  inde_var[, which(grepl("ROAD", names( inde_var) ))]
 ...
 # roadsonly_in[,for5000m] = 0 # don't need 5000, so let them be 0 and put them in front, for road ring 0.
  othervar = inde_var[, -which(grepl("ROAD", names( inde_var) ))]
