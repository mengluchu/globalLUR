...
```{r}
library(gstat)
#st_as_sf(merged_mr, coords = c("LONGITUDE","LATITUDE"))
...
```{r residual}
response_predictor= globalLUR::sampledf(merged_mr,fraction = 0.8, country2digit = "CN" ,grepstring_rm = "ID|countryfullname")
test = response_predictor$test
training = response_predictor$training
inde_varexp = response_predictor$inde_var
...
rf_error = predictions(predict(rf, data =inde_var_test  )) - inde_var_test$value_mean
rferror_df = data.frame(error = rf_error, LATITUDE = inde_var_test$LATITUDE,LONGITUDE = inde_var_test$LONGITUDE)
error_sf  = st_as_sf(rferror_df, coords = c("LONGITUDE","LATITUDE"))
 
grd_sp <- as_Spatial(error_sf)
dt.vgm = variogram(error~1,grd_sp,cutoff = 1)
plot(dt.vgm)
```
...
response_predictor= globalLUR::sampledf(merged_mr,fraction = 0.8, country2digit = "DE" ,grepstring_rm = "ID|countryfullname")
...
library(party)
 ...
 ctree_LUR(inde_var, y_varname= c("day_value"), training=training, test= test, grepstring ="ROAD|pop|temp|wind|Rsp|OMI|eleva|coast|I_1|Tropo|LON|LAT" )
...
for (i in 2:5)
{set.seed(i)
a= globalLUR::sampledf(merged_mr,fraction = 0.8, "DE" ,grepstring_rm = "ID|countryfullname")
test = a$test
training = a$training
