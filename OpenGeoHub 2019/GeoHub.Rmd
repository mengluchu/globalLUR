...
fit_control <- trainControl(## 10-fold CV
                           method = "cv",
                           number = 10)
model_rf = train(value_mean ~ ., data = inde_var_train, method='rf', trControl = fit_control) # mtry
plot(model_rf)
...
```{r XGBOOST}
  ...
  xgb.train (params = params, data = dtrain, nrounds = 79, watchlist = list(val=dtest,train=dtrain), print.every.n = 10, early.stop.round = 10, maximize = F , eval_metric = "error")
  ...
  nrounds = 1000
  Gamma = 8 
  bst <- xgboost(data = dfmatrix, label = y_train, max_depth = max_depth, eta = eta, nthread = nthread, nrounds = nrounds, Gamma = Gamma, verbose = 0)
  tail(bst)
  x_test%>%select(-day_value)->var_tet  
  xgbpre = predict(bst, as.matrix(var_tet))
